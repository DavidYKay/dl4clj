...
(defn make-builder []
  (doto (new NeuralNetConfiguration$Builder)
    (.seed seed)
    ...
    (.momentum 0.9)
    (.list 6)
    (.layer 0 (doto (new ConvolutionLayer$Builder 5 5)
                (.nIn nChannels)
                (.stride 1 1)
                (.nOut 20)
                (.activation "identity")
                (.build)))
    (.layer 1 (doto (SubsamplingLayer$Builder. SubsamplingLayer$PoolingType/MAX)
                (.kernelSize 2 2)
                (.stride 2 2)
                (.build)))
...
(let [builder (make-builder)
      ...
      ^MultiLayerConfiguration conf (.build builder)
